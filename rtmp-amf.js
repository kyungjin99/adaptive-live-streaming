...
function amfType(o) {
  ...
  if (jsType === 'number') {
    if (parseInt(o, 10) === o) return 'integer';
    return 'double';
  }
...
function amf3decArray(buf) {
  ...
  if (count.value % 2 === 1) throw new Error('This is a reference to another array, which currently we dont support!');
...
function decodeAmf0Data(dbuf) {
  ...
  if (cmd) {
    ...
    if (rtmpDataCode[cmd.value]) {
      rtmpDataCode[cmd.value].forEach((n) => {
        if (buffer.length > 0) {
...
function decodeAMF0Cmd(dbuf) {
  ...
  if (rtmpCmdCode[cmd.value]) {
    rtmpCmdCode[cmd.value].forEach((n) => {
      if (buffer.length > 0) {
        let r = amf0DecodeOne(buffer);
        buffer = buffer.slice(r.len);
        resp[n] = r.value;
...
function encodeAMF0Data(opt) {
  ...
  if (rtmpDataCode[opt.cmd]) {
    rtmpDataCode[opt.cmd].forEach((n) => {
      if (opt.hasOwnProperty(n)) {
        data = Buffer.concat([data, amf0EncodeOne(opt[n])]);
      }
    });
